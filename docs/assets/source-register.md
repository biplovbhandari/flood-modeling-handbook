...
This register records the provenance, description, source statement, reuse status, access date, and modification history for the handbook's nine file-based visuals.
Each visual is an original handbook asset, and its reuse terms remain pending a publication-rights decision.
...
- **Description:** An original process diagram showing a prepared reach network, realized terrain source, and realized land-cover source and lookup flowing through geometry, terrain, and roughness transformations into model artifacts, a model record, and an identity-addressed generation.
...
- **Modification notes:** Created as a code-native SVG for the handbook.
  ...
  A dashed grouping rectangle spans x=590 through x=875 and y=120 through y=670 to enclose the artifact outputs and model record as one publication set.
  The geometry artifact reaches the model record through x=865 and enters its right side at y=500, while the raster artifact enters the model record vertically at x=790.
  The publication-set route begins at `(875, 645)`, runs through x=890, and enters the model generation at `(905, 265)`.
  A separate dashed vertical route connects the generation to the evidence gate without crossing another box.
  The file includes an accessible title and description, and color is paired with text labels and layout.

### VIS-007: Adaptive discharge proposal and measured verdict

- **File:** `assets/nd-adaptive-selection.svg`
- **Description:** An original response-curve diagram showing a reference point, monotone measured and extrapolated curve, floor and ceiling crossings, shaded discharge window, proposed discharge, predicted response, and a later measurement above the upper threshold.
...
### VIS-008: Downstream-to-upstream stage transfer

- **File:** `assets/kwse-stage-transfer.svg`
- **Description:** An original two-domain schematic showing physical flow downstream, hydraulic scenario dependency upstream, overlapping upstream and downstream grids, stage-transfer geometry, one downstream depth-plus-terrain WSE sample, one upstream cell-specific transfer point, fixed-discharge inflow, outer-edge outflow spans, nominal scenario labeling, and responsibility boundaries.
  The grids have visibly different origins and resolutions.
  The learner should notice that one shared map query coordinate lies inside distinct upstream and downstream containing cells, that the downstream sampled cell center and value are separate from the upstream transfer cell, and that the nominal stage labels the scenario while downstream raster values set pointwise transferred stages.
